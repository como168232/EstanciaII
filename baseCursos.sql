...
CREATE INDEX `idBeneficio_idx` ON `cursos`.`Transaccion` (`idBeneficio` ASC) INVISIBLE;
...
CREATE TABLE IF NOT EXISTS `cursos`.`InformacionAlumno` (
  ...
  PRIMARY KEY (`idInformacionAlumno`),
  CONSTRAINT `idAlumno`
...
CREATE INDEX `idPrueba_idx` ON `cursos`.`Pregunta` (`idPrueba` ASC) VISIBLE;
